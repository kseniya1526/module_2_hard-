...
def get_password(n):
    ...
    passcode.update({14: 1611325212343114105968, 15: 1214114232133124115106978})
    passcode.update({16: 1317115262143531341251161079, 17: 11621531441351261171089})
    passcode.update({18: 12151811724272163631545414513612711810, 19: 118217316415514613712811910})
...
pairofnumber1 = list(range(1, n))
pairofnumber2 = list(range(1, n))
pairs = []
result = ''

for i in pairofnumber1:
    for j in pairofnumber2:
        pn1 = i  # pairofnumber1[i]
        pn2 = j  # pairofnumber2[j]
        if pn1 >= pn2:
            continue
        else:
            multiple = n % (pn1 + pn2)
